...
# Set Tesseract path - using your exact path
...
if os.path.exists(TESSERACT_PATH):
    ...
    # Try alternative common paths as fallback
    alt_paths = [
...
# Set Poppler path - using your exact path
...
try:
    ...
    if os.path.exists(index_path) and os.path.exists(chunks_path):
        ...
        print("⚠️ HEDIS index files not found. Create them first.")
        index = None
        hedis_chunks = []
except Exception as e:
    print(f"⚠️ Error loading HEDIS files: {e}")
    index = None
    hedis_chunks = []
...
def extract_text_from_scanned_pdf(file_bytes):
    ...
    try:
        ...
        if os.path.exists(POPPLER_PATH):
            ...
            # Try system Poppler as fallback
...
def structure_medical_data(raw_text):
    ...
    raw_output = ""  # Initialize variable
...
def analyze_care_gaps(patient_data, hedis_rules):
    ...
    prompt = f"""
    You are an experienced Healthcare Quality Improvement Assistant specializing in HEDIS preventive care measures.

    TASK:
    Review the patient data and compare it ONLY with the supplied HEDIS guidelines.

    OBJECTIVE:
    Identify preventive care gaps that are supported by BOTH:
    1. The patient's medical record.
    2. The supplied HEDIS guideline.

    IMPORTANT RULES:

    • Use ONLY information explicitly available in the patient data.
    • Use ONLY the supplied HEDIS guideline.
    • Do NOT assume medications, diagnoses, screenings, vaccinations, or procedures.
    ...
    • If a HEDIS measure cannot be evaluated because information is unavailable, state that clearly.
    • Prioritize clinically important care gaps first.

    OUTPUT FORMAT

    Care Gaps Found

    For each care gap provide:
    ...
    Limit to a maximum of FIVE care gaps.

    If no significant care gaps are identified, write exactly:

    "No significant HEDIS care gaps identified from the available records."
    ...
    • Suggested actions must be general recommendations such as discussing findings with a healthcare provider.
